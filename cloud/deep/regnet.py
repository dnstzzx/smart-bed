...
class RegNet(nn.Module):
    def __init__(self, regnet_type, num_classes=1000):
        ...
        '''
        self.conv1 = nn.Sequential(nn.Conv2d(3, 24, 1),
                                   nn.BatchNorm2d(24),
                                   nn.ReLU(),
        ...
        self.fc2 = nn.Sequential(nn.Dropout(0.6),
                                 nn.Linear(368, 256),
                                 nn.BatchNorm1d(256),
                                 nn.ReLU(),
                                 nn.Dropout(0.6),
                                 ...
                                 nn.Linear(32, 1),
                                 nn.Sigmoid())
        self.fc3 = nn.Sequential(nn.Dropout(0.6),
                                 nn.Linear(368, 256),
                                 nn.BatchNorm1d(256),
                                 nn.ReLU(),
                                 nn.Dropout(0.6),
...
net = RegNetY_32GF().cuda()
criterion = nn.BCELoss()
optimizer = optim.Adam(net.parameters(), lr=0.001)



x = torch.randn(2, 3, 224, 224).float().cuda()
y1, y2 = net(x)


d1, d2 = torch.cat([d1, d3], 0).float().cuda(), torch.cat([d2, d4], 0).float().cuda()
print (d1.shape)
optimizer.zero_grad()
loss = criterion(y1, d1)
loss2 = criterion(y2, d2)
loss3 = loss + loss2
